...
fn test_pplx1embed() -> Result<()> {
    ...
    insta::assert_yaml_snapshot!("pplx1embed_cpu_batch", embeddings_batch, &matcher);
    ...
    insta::assert_yaml_snapshot!("pplx1embed_cpu_single", embeddings_single, &matcher);
...
#[test]
#[serial_test::serial]
fn test_pplx1embed_quantization() -> Result<()> {
    let model_root = std::path::PathBuf::from("/traindata/markus/pplx-embed-1-0.6b");
    let tokenizer = load_tokenizer(&model_root)?;
