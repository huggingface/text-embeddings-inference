...
impl PPLX1EmbedModel {
    ...
    pub fn forward(&self, batch: Batch) -> Result<(Option<Tensor>, Option<Tensor>)> {
        ...
        let (input_ids, position_ids, input_lengths, attention_bias) = if batch_size > 1 {
            ...
            // No attention bias for single sequences (bidirectional attention)
            (input_ids, position_ids, input_lengths, None)
        };

        // REMOVED: Causal masking logic - attention_bias now only contains padding mask
        ...
        let pooled_embeddings = if has_pooling_requests {
            ...
            if let Some(embeddings) = pooled {
                let quantized = embeddings
                    .tanh()? // Apply tanh: [-1, 1]
                    .affine(127.0, 0.0)? // Scale: [-127, 127]
