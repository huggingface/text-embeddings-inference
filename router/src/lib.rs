...
pub async fn run(
    ...
    if config.model_type().is_some_and(|t| t == "qwen2")
        && config
            .auto_map
            .as_ref()
    ...
    let position_offset = match config.model_type() {
        Some("xlm-roberta") | Some("camembert") | Some("roberta") => config.pad_token_id + 1,
        _ => 0,
    };
    ...
    let base_input_length = match st_config {
        ...
        None => {
            ...
            let mut length = config.max_position_embeddings - position_offset;
            if let Some(window) = config.sliding_window() {
                length = length.min(window);
            }
            length
        }
    };
    ...
    let dtype = if dtype.is_none() && config.model_type().is_some_and(|t| t == "gemma3_text") {
        DType::Float32
    } else {
...
impl ModelConfig {
    ...
    fn sliding_window(&self) -> Option<usize> {
        self.sliding_window
            .or_else(|| self.text_config.as_ref()?.sliding_window)
    }
}
