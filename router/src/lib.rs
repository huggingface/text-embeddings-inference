...
fn get_backend_model_type(
    ...
    for arch in &config.architectures {
        ...
        // Some causal LM models can also be used as classifiers if the `id2label` and `label2id` mapping are provided in the `config.json`, as shown with Qwen3ForCausalLM.
        if arch.ends_with("CausalLM") && config.id2label.is_some() && config.label2id.is_some() {
            tracing::warn!("Provided `--model-id` is likely an CausalLM, but the `config.json` contains the architecture `xxxCasualLM` and it contains the `id2label` and `label2id` mapping, so `{arch}` architecture will be presumed to be a classifier.");
