...
It also supports modular Sentence Transformers cross-encoder re-rankers whose score is produced by a post-pooling head (`Transformer -> Pooling -> Dense -> ... -> Dense`) rather than by a `*ForSequenceClassification` head, such as the `cross-encoder/ettin-reranker-*` family.
...
| Re-Ranking         | ModernBert  | [cross-encoder/ettin-reranker-17m-v1](https://huggingface.co/cross-encoder/ettin-reranker-17m-v1) |
